...
const Header = ({ wallpaper }: WallpaperProp) => {
  const backgroundImage = wallpaper.backdrop_path || wallpaper.profile_path;

  return (
    <div
      style={{
        background: `linear-gradient(rgba(0,0,0,.4),rgba(0,0,0,.7),rgba(0,0,0,.9)), url(https://image.tmdb.org/t/p/original/${backgroundImage})`,
        backgroundSize: "cover",
        backgroundPosition: "center",
      }}
      className="text-white bg-transparent shadow-[0_4px_20px_rgba(0,255,0,0.6)] h-[50vh] w-[70vw] -translate-y-40 translate-x-10 opacity-60 hover:opacity-100
    ...
    >
      ...
      <p className="flex flex-row text-white p-2 items-center justify-start translate-x-8">
        <img
          src={anime}
          className="w-10 h-10 rounded-lg p-1 mr-4"
          alt="anime"
        />
        {(wallpaper.media_type || "Unknown").toUpperCase()}
      </p>
